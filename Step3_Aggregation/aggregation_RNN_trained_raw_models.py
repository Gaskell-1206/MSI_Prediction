# Run RNN aggregation use raw MIL models
...
def main(args):
    ...
    version_name = f'{model_name}_bs{args.batch_size}_lr{args.learning_rate}'
    ...
    ckpt_file_path = glob.glob(os.path.join(file_path,'*.ckpt'))[0]
    
    # prepare k tile_ids
    df_train = pd.read_csv(file_path / f'Tiles_level_Train_{version_name}_output.csv')
    ...
    df_val = pd.read_csv(file_path / f'Tiles_level_Val_{version_name}_output.csv')
    ...
    df_test = pd.read_csv(file_path / f'Tiles_level_Test_{version_name}_output.csv')
    ...
    #make model
    embedder = ResNetEncoder(ckpt_file_path)
    ...
    # fconv.write('epoch,train.loss,train.fpr,train.fnr,val.acc,val.loss,val.fpr,val.fnr\n')
    # fconv.close()
    
    # open output file
    version_name = f'RNN_tiles_level_{model_name}_bs{args.batch_size}_lr{args.learning_rate}_w{args.weights}_k{args.k}_output'
    # logger
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, path):
        ...
        if args.model_name == 'densenet':
            self.fc = model.model.classifier
